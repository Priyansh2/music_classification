			...
			for line in lyrics:
				if line.split("\n")[0].strip():
					...
					for w,trans_w,lang in output_[:-1]:
						...
						trans_word_str+=trans_w+" "
					word_str+=output_[-1][0]+"$$$$$"+output_[-1][1]+"$$$$$"+output_[-1][2]
					trans_word_str+=output_[-1][1]
					li_tags.append(output_[-1][2])
					rem_str = word_str
					trans_rem_str = trans_word_str
					if prev:
						for i in range(l):
							trans_lyrics_v2.append((trans_word_str.split()[i]+"\n"))
							trans_lyrics.append(word_str.split()[i]+"\n")
						trans_rem_str=" ".join(x for x in trans_word_str.split()[l:])
						rem_str = " ".join(x for x in word_str.split()[l:])
						prev=False
						prev_str=None
					trans_lyrics_v2.append(trans_rem_str+"\n")
					trans_lyrics.append(rem_str+"\n")
				else:
					trans_lyrics_v2.append("\n")
					trans_lyrics.append("\n")
			#break
			#print(trans_lyrics)
			trans_all_lyrics.append((file_path,trans_lyrics_v2))
			all_lyrics.append((file_path,trans_lyrics))
		#break
	return corpus,li_tags,all_lyrics,trans_all_lyrics
...
'''corpus,li_tags,all_lyrics,trans_all_lyrics = LI()
...
def write_lyrics_to_file(all_lyrics,trans_all_lyrics):
	#global tagged_corpus
	global csnli_path
	tagged_corpus=''
	for m,n in zip(all_lyrics,trans_all_lyrics):
		original_file_path,trans_lyrics,trans_lyrics_v2 = m[0],m[1],n[1]
		...
		#print(trans_lyrics)
		for line1,line2 in zip(trans_lyrics,trans_lyrics_v2):
			str1=''
			if line2.split("\n")[0].strip():
				tagged_corpus+=line1
				str1+=line2
				for delim in delims:
					if delim in str1 and delim!="\n":
...
'''write_lyrics_to_file(all_lyrics,trans_all_lyrics)'''
